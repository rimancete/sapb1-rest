...
export class HanaCurrencyQuoteService extends DatabaseService<any> {
	...
	updateRetry(currency: string, date: string): Promise<DatabaseResponse<any[]>> {
	...
	getIntegrationValues(): Promise<DatabaseResponse<any>> {
		...
		SELECT (
				...
				WHERE CurrencyQuote."U_ALFA_Integrated" = 'Y'
		...
		WHERE CurrencyQuote."U_ALFA_Integrated" = 'N') INTO NOTIMPORTED FROM DUMMY;
