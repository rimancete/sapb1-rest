...
export class HanaBusinessPartnersService extends DatabaseService<any> {
  ...
  getNotIntegrated(): Promise<SupplierClient[]> {
    return this.execute(`SELECT 
                              ...
                              AND City."U_ALFA_Integrated" = 'Y'
                                ...
                                SELECT 
                                  "CardCode",
                                MAX(IFNULL("Block",'0')) AS "Block",
                                MAX(IFNULL("StreetNo",'0')) AS "StreetNo"
                              ...
                              WHERE 
                                PN."U_ALFA_Integrated" = 'N'
                              AND PN."U_ALFA_Retry" < 3  
                                -- AND PN."CreateDate" >= '2020-08-01'
                                `);
  }
  ...
  async updateRetry(record: SupplierClient): Promise<DatabaseResponse<any>> {
    const query = `UPDATE ${this.databaseName}.OCRD SET "U_ALFA_Retry" = "U_ALFA_Retry" + 1 WHERE "CardCode" = '${record.Code}'`;
    return await this.execute(query);
  }
  ...
  async getAllAdress(partner: string):  Promise<SupplierClientAddresses[]> {
    ...
    return this.execute(`SELECT 
                            CONCAT(CONCAT(T1."LineNum",' - '),T1."CardCode")        AS "Code",
                            T2."Code" 		    AS "CityCode",
                            T1."Street"           AS "Street",
                          -- T1."City"             AS "City",
  ...
  async getBranch(branch: number): Promise<any> {
    ...
    const query = `SELECT Branch."BPLId" FROM ${this.databaseName}.OBPL Branch WHERE Branch."BPLId" = '${branch}'`;
