...
export class HanaContractService extends DatabaseService<any> {
    ...
    OA."AbsID" 		-- contrato
   ,OA."U_ALFA_FilialDestino" AS "BPLId"     -- Companycode 
   ,OA."StartDate"		-- startdate
   ,OA."EndDate"       -- enddate
   ,OA."Status"		-- status
   ,OA."BpCode"		-- ProviderCode
   ,TO_VARCHAR(OA."Remarks")	AS "Remarks"	-- Observações	
   ,OA."U_ALFA_Conctract_Type" AS "Type"

FROM ${this.databaseName}.OOAT OA 
LEFT JOIN ${this.databaseName}.OBPL Company ON "MainBPL" = 'Y'
LEFT JOIN ${this.databaseName}.PQT1 pqt ON pqt."AgrNo" = OA."AbsID"
LEFT JOIN ${this.databaseName}.OPQT opq ON opq."DocEntry" = pqt."DocEntry"
WHERE OA."U_ALFA_IntegrateF1" = 'Y' 
AND OA."U_ALFA_Integrated" = 'N' 
AND OA."Status" = 'A'
AND OA."U_ALFA_Retry" < 3
	...
	async updateRetry(record: any): Promise<DatabaseResponse<any>> {
		const query = `UPDATE ${this.databaseName}.OOAT SET "U_ALFA_Retry" = "U_ALFA_Retry" + 1 WHERE "AbsID" = '${record.AbsID}'`;
		return await this.execute(query);
	}
