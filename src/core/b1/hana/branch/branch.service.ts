...
export class HanaBranchService extends DatabaseService<any> {
	...
	async updateRetry(record: any): Promise<DatabaseResponse<any>> {
		const query = `UPDATE ${this.databaseName}.OBPL SET "U_ALFA_Retry" = "U_ALFA_Retry" + 1 WHERE "BPLId" = '${record.Code}'`;
		return await this.execute(query);
  }
  
	async getBPLId(id: string): Promise<DatabaseResponse<Branch[]>> {
	...
	getIntegrationValues(): Promise<DatabaseResponse<Branch[]>> {
        ...
        SELECT (SELECT COUNT(*) FROM ${this.databaseName}.OBPL WHERE "MainBPL" = 'N') INTO TOTAL_BRANCH FROM DUMMY;
        
        SELECT (SELECT COUNT(*) FROM ${this.databaseName}.OBPL WHERE "U_ALFA_Integrated" = 'Y' AND "MainBPL" = 'N') INTO IMPORTED FROM DUMMY;
        
        SELECT (SELECT COUNT(*) FROM ${this.databaseName}.OBPL WHERE "U_ALFA_Integrated" = 'N' AND "MainBPL" = 'N') INTO NOTIMPORTED FROM DUMMY;
