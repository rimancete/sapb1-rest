...
export class HanaItemPriceService extends DatabaseService<any> {
    ...
    SELECT
			...
			MAX(Journal."DocDate")				AS	"PriceDate",
			Cost."AvgPrice"						AS	"Price",
		...
		FROM
			...
			INNER JOIN ${this.databaseName}."OITW" Cost
			ON Cost."ItemCode" = Journal."ItemCode"
			AND Cost."WhsCode" = Journal."LocCode"
			
		WHERE
			Journal."LocType" = 64
		AND
			Journal."U_ALFA_Retry" < 3
	...
	updateRetry(itemCode: string, date: string, warehouse: string): Promise<DatabaseResponse<any>> {
		const query = (`
		UPDATE 
			${this.databaseName}.OIVL Journal
		SET
