...
export class HanaStateService extends DatabaseService<any> {
	...
	updateRetry(record: State): Promise<DatabaseResponse<any>> {
		return this.exec(`UPDATE ${this.databaseName}.OCST SET "U_ALFA_Retry" = "U_ALFA_Retry" + 1 WHERE "Code" = '${record.Code}'`);
	}
