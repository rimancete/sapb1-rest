...
export class HanaItemGroupService extends DatabaseService<any> {
    ...
    WHERE
      ...
      AND
        ItemGroup."U_ALFA_Code" is not null
      AND
        ItemGroup."U_ALFA_IntegracaoSF" = 'Y'
	  AND 
			ItemGroup."U_ALFA_Retry" < 3
		`);
	}
	...
	async updateRetry(record: any): Promise<DatabaseResponse<any>> {
		const query = `UPDATE ${this.databaseName}.OITB SET "U_ALFA_Retry" = "U_ALFA_Retry" + 1 WHERE "ItmsGrpCod" = '${record.Code}'`;
