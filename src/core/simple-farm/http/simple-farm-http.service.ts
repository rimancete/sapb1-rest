...
export class SimpleFarmHttpService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.queue = new Queue(
      async (data, callback) => {
        try {
          const result = await this.axiosRef.post<SimpleFarmResponse>(
            data.path,
            data.data,
            this.config,
          );
          ...
          if (result.data) {
              ...
              if (result.data.Executed) {
                ...
                callback({
                  ...
                  data: new Exception({
                    code: 'SF001',
                    message:
                      'Erro ao incluir/atualizar o registros no SimpleFarm',
                    request: data,
              ...
              if (result.data.HasErrors) {
                callback({
                  ...
                  data: new Exception({
                    code: 'SF001',
                    message:
                      'Erro ao incluir/atualizar o registro no SimpleFarm',
                    request: data,
                    response: result.data,
                  }),
                });
            ...
            callback({
              error: true,
              data: new Exception({
                code: result.status ? result.status.toString() : 'undefined',
  ...
  async post(
    ...
    data: any,
    batch: boolean = false,
  ): Promise<SimpleFarmResponse> {
    const p = new Promise<SimpleFarmResponse>((resolve, reject) => {
      this.queue.push({ path, data, batch }, result => {
        if (result.error) {
          reject(result.data);
        } else {
          resolve(result.data);
